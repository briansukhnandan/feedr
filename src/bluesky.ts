...
import type { FeedItem, FeedMedia, FeedPost, PublicationReceipt, Publisher, PublishContext } from "./types.js";
...
export class BlueskyPublisher implements Publisher {
  ...
  constructor(options: BlueskyPublisherOptions) {
    ...
    this.agent = new AtpAgent({ service: options.service ?? "https://bsky.social" });
  }

  async publish(item: FeedItem, context: PublishContext): Promise<PublicationReceipt> {
    await this.prepare();
    const segments = item.thread?.length ? item.thread : [{ text: item.text, url: item.url, media: item.media }];
    ...
    if (!last) throw new Error("A Bluesky item must contain at least one post.");
    return { id: last.uri, url: `https://bsky.app/profile/${this.agent.session?.did}/post/${last.uri.split("/").at(-1)}`, publishedAt: new Date(), metadata: { rootUri: root?.uri } };
  }
  ...
  private async postSegment(segment: FeedPost, root: StrongRef | undefined, parent: StrongRef | undefined, context: PublishContext): Promise<StrongRef> {
    ...
    if (segment.media?.length) record.embed = await this.createImagesEmbed(segment.media, context);
  ...
  private async createImagesEmbed(media: FeedMedia[], context: PublishContext): Promise<AppBskyFeedPost.Record["embed"]> {
    const images = await Promise.all(media.slice(0, 4).map(async (image) => {
      const bytes = image.data ?? await this.fetchMedia(image, context.signal);
      const upload = await this.agent.uploadBlob(bytes, { encoding: image.mimeType });
      return {
        alt: image.alt,
  ...
  private async fetchMedia(media: FeedMedia, signal?: AbortSignal): Promise<Uint8Array> {
    if (!media.url) throw new Error("Feed media requires either data or url.");
    const response = await fetch(media.url, { signal });
    if (!response.ok) throw new Error(`Could not fetch media (${response.status}) from ${media.url}.`);
